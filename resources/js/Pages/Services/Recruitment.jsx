...
import { CheckCircle2, ArrowRight, Users, AlertTriangle, ChevronRight, HelpCircle } from 'lucide-react';

export default function Recruitment() {
  const service = servicesData.find(s => s.id === 'recruitment');

  return (
    <Layout
      title="Recruitment Services & Talent Acquisition"
      description="Sector-focused recruitment and executive sourcing across Automobile, Real Estate, Construction, Infrastructure, Media & Manufacturing."
    >
      {/* SERVICE HERO */}
      ...
      <section className="py-20 bg-slate-900/40 relative">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="max-w-2xl mb-12">
            <span className="text-xs font-bold uppercase tracking-widest text-amber-400">CHALLENGES SOLVED</span>
            <h2 className="text-3xl font-extrabold text-white tracking-tight mt-1">
              Hiring Friction That We Eliminate
            </h2>
          </div>
      ...
      <section className="py-24 bg-slate-950">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="max-w-2xl mb-16">
            <span className="text-xs font-bold uppercase tracking-widest text-amber-400">EXECUTION WORKFLOW</span>
            <h2 className="text-3xl font-extrabold text-white tracking-tight mt-1">
              Our Sourcing &amp; Screening Process
            </h2>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {service.process.map((p) => (
              <div key={p.step} className="p-6 rounded-2xl bg-slate-900 border border-slate-800 space-y-3 relative">
